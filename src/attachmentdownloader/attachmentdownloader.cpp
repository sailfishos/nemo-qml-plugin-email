...
{
    qCDebug(lcMessaging) << "Checking for attachments to download";
...
{
    qCDebug(lcMessaging) << "Online state changed:" << online;
    if (online) {
        processNext();
...
void AttachmentDownloader::activityChanged(QMailServiceAction::Activity activity)
{
    ...
    case QMailServiceAction::Failed:
        qCWarning(lcMessaging) << Q_FUNC_INFO << "Attachment download failed, account: " << m_account
    ...
    case QMailServiceAction::Successful:
        qCDebug(lcMessaging) << Q_FUNC_INFO << "Attachment download finished for account" << m_account;
        break;
    ...
    qCDebug(lcMessaging) << Q_FUNC_INFO << "Attachment download queue length is now" << m_locationQueue.size();
}
...
void AttachmentDownloader::autoDownloadAttachments(const QMailMessageId &messageId)
{
    ...
    for (auto &location : message.findAttachmentLocations()) {
        ...
        if (isEmailPart(attachmentPart) && !attachmentPart.contentAvailable()) {
            ...
            if (enqueue(location)) {
                qCDebug(lcMessaging) << Q_FUNC_INFO << "Auto download attachment for:" << location.toString(true)
...
void AttachmentDownloader::processNext()
{
    if (!m_locationQueue.isEmpty() && m_networkConfiguration.isOnline() && !m_action.isRunning()) {
        ...
        qCDebug(lcMessaging) << Q_FUNC_INFO << "Executing next attachment download action for account" << m_account;
...
{
    qCDebug(lcMessaging) << Q_FUNC_INFO << "Canceling and requeing attachment download action for account" << m_account;
