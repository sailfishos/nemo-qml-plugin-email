...
QString AttachmentListModel::attachmentPath(const QMailMessage &message,
                                            const QMailMessagePartContainer::Location &attachmentLocation)
{
    if (!message.contains(attachmentLocation))
        return QString();

    const QMailMessagePart &sourcePart = message.partAt(attachmentLocation);

    // TODO: create a method in QMF to get the filename generated by
    // writeBodyTo() as used in the agent.
    const QString attachmentPath = EmailAgent::instance()->attachmentPath(message.parentAccountId(), sourcePart) + "/" + EmailAgent::instance()->attachmentName(sourcePart);
...
void AttachmentListModel::resetModel()
{
    ...
    connect(m_attachmentFileWatcher, &QFileSystemWatcher::directoryChanged, this, [this]() {
        ...
        for (const QMailMessagePart::Location &location : locations) {
            QString path = attachmentPath(m_message, location);
            if (!path.isEmpty()) {
                onAttachmentPathChanged(location.toString(true), path);
            }
        }
    });
    ...
    if (m_messageId.isValid()) {
        ...
        for (const QMailMessagePart::Location &location : locations) {
            ...
            const QString dlFolder = EmailAgent::instance()->attachmentPath(m_message.parentAccountId(), item->part);
            ...
            if (item->status == EmailAgent::Unknown) {
                item->url = QUrl::fromLocalFile(attachmentPath(m_message, location)).toString();
