...
bool EmailAgent::saveAttachmentToDownloads(const QMailMessageId &messageId, const QString &attachmentLocation)
{
    ...
    if (message.contains(location)) {
        const QMailMessagePart &attachmentPart = message.partAt(location);
        const QString attachmentFolder = attachmentPath(message.parentAccountId(),
                                                        attachmentPart);
        // TODO: create a method in QMF to get the filename generated by
        // writeBodyTo() as used later.
        const QString attachmentPath = attachmentFolder + "/" + attachmentName(attachmentPart);
        ...
        if (attachmentFile.exists()) {
            ...
            QString path = attachmentPart.writeBodyTo(attachmentFolder);
            if (!path.isEmpty()) {
                emit attachmentPathChanged(attachmentLocation, path);
                updateAttachmentDownloadStatus(attachmentLocation, Downloaded);
                return true;
            } else {
                qCDebug(lcEmail) << "ERROR: Failed to save attachment file to location:" << attachmentFolder;
...
QString EmailAgent::attachmentPath(const QMailAccountId &accountId,
                                   const QMailMessagePart &part) const
{
    return QStandardPaths::writableLocation(QStandardPaths::DownloadLocation)
        + "/mail_attachments"
        + "/" + QString::number(accountId.toULongLong())
        + "/" + part.location().toString(true);
}
